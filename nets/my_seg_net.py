...
slim = tf.contrib.slim
        ...
        # 
        # loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,labels=correct_label))
...
def load_inception(inputs, num_classes):

    with slim.arg_scope(inception_utils.inception_arg_scope()):
        net, end_points = inception_v4.inception_v4_base(inputs)
        layer1_out = end_points['InceptionV4/Conv2d_1a_3x3/Relu:0']
        layer2_out = end_points['InceptionV4/Conv2d_2b_3x3/Relu:0']
        layer3_out = end_points['InceptionV4/Mixed_3a/concat:0']
        layer4_out = end_points['InceptionV4/Mixed_4a/concat:0']
        layer5_out = end_points['InceptionV4/Mixed_5e/concat:0']
        layer6_out = end_points['InceptionV4/Mixed_6h/concat:0']
        layer7_out = end_points['InceptionV4/Mixed_7d/concat:0']

    deconv7 = tf.layers.conv2d_transpose(layer7_out,1024,4,2,'VALID')
    deconv7 = tf.layers.batch_normalization(deconv7)
    end_points["Deconv7"] = deconv7

    #conv6 = tf.layers.conv2d(vgg_layer6_out,1,1, trainable=False)
    add1 = tf.add(deconv7,layer6_out)
    #add1 = tf.layers.batch_normalization(add1)
    deconv6 = tf.layers.conv2d_transpose(add1,384,3,2,'VALID')
    deconv6 = tf.layers.batch_normalization(deconv6)
    ...
    #conv5 = tf.layers.conv2d(vgg_layer5_out,1,1, trainable=False)
    add2 = tf.add(deconv6,layer5_out)
    #add2 = tf.layers.batch_normalization(add2)
    deconv5 = tf.layers.conv2d_transpose(add2,192,4,2,'VALID')
    deconv5 = tf.layers.batch_normalization(deconv5)
    ...
    #conv4 = tf.layers.conv2d(vgg_layer4_out,1,1, trainable=False)
    add3 = tf.add(deconv5,layer4_out)
    #add3 = tf.layers.batch_normalization(add3)
    deconv4 = tf.layers.conv2d_transpose(add3,64,20,4,'VALID')
    deconv4 = tf.layers.conv2d_transpose(deconv4,num_classes,1,1,'SAME')
    end_points["deconv4"] = deconv4

    return deconv4, end_points
...
fcn_inception.default_image_height = 1280#448#1280
fcn_inception.default_image_width = 1920#448#1920
